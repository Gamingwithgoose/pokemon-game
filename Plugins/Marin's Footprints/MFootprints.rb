...
class Sprite_Character
  ...
  TERRAIN_SAND = 3
  #TERRAIN_SNOW = ? ADD THIS IN WHEN SNOW TERRAIN TAG IS DEFINED AND UNCOMMENT THE REST OF LINE 93                                             
  ...
  def update
    ...
    @old_x ||= @character.x
    @old_y ||= @character.y
    @oldDirection ||= @character.direction
    if (@character.x != @old_x || @character.y != @old_y) && !["", "nil"].include?(@character.character_name)
      if @character == $game_player && $PokemonGlobal.dependentEvents &&
        ...
        make_steps = [2,1,0].any? do |e|
          ...
          next $data_tilesets[tilesetid].terrain_tags[tile_id] == TERRAIN_SAND # || $data_tilesets[tilesetid].terrain_tags[tile_id] == TERRAIN_SNOW
        end
      end
      if make_steps
        ...
        dirs = [nil,"DownLeft","Down","DownRight","Left","Still","Right","UpLeft",
            "Up", "UpRight"]
        #check for tire tracks or footprints
        if @character_name.include? "bike" # == $game_player && $PokemonGlobal.bicycle
          bike = true
        elsif @character_name.include? "BIKE"
          bike = true
        else
          bike = false
        end
        #if tire tracks, check for turning, else print footprint direction
        if bike
          if @oldDirection != @character.direction 
            if @oldDirection == 2 #DOWN
              if character.direction == 4 #LEFT
                fstep.bmp("Graphics/Characters/MSteps/stepsDown#{dirs[@character.direction]}Bike")
              elsif character.direction == 6 #RIGHT
                fstep.bmp("Graphics/Characters/MSteps/stepsDown#{dirs[@character.direction]}Bike")
              else
                fstep.bmp("Graphics/Characters/MSteps/steps#{dirs[@character.direction]}Bike")
              end
            elsif @oldDirection == 4 #LEFT
              if character.direction == 2 #DOWN
                fstep.bmp("Graphics/Characters/MSteps/stepsUpRightBike")
              elsif character.direction == 8 #UP
                fstep.bmp("Graphics/Characters/MSteps/stepsDownRightBike")
              else
                fstep.bmp("Graphics/Characters/MSteps/steps#{dirs[@character.direction]}Bike")
              end
            elsif @oldDirection == 6 #RIGHT
              if character.direction == 2 #DOWN
                fstep.bmp("Graphics/Characters/MSteps/stepsUpLeftBike")
              elsif character.direction == 8 #UP
                fstep.bmp("Graphics/Characters/MSteps/stepsDownLeftBike")
              else
                fstep.bmp("Graphics/Characters/MSteps/steps#{dirs[@character.direction]}Bike")
              end
            elsif @oldDirection == 8 #UP
              if character.direction == 4 #LEFT
                fstep.bmp("Graphics/Characters/MSteps/stepsUP#{dirs[@character.direction]}Bike")
              elsif character.direction == 6 #RIGHT
                fstep.bmp("Graphics/Characters/MSteps/stepsUp#{dirs[@character.direction]}Bike")
              else
                fstep.bmp("Graphics/Characters/MSteps/steps#{dirs[@character.direction]}Bike")
        ...
        if @character_name.include? "bike" #== $game_player && $PokemonGlobal.bicycle
          x = BIKE_X_OFFSET
          y = BIKE_Y_OFFSET
        elsif @character_name.include? "BIKE" #== $game_player && $PokemonGlobal.bicycle
          x = BIKE_X_OFFSET
          y = BIKE_Y_OFFSET
        else
          x = WALK_X_OFFSET
